...
export default function CodeInterface({ deploymentId, workflowName, onBack }: CodeInterfaceProps) {
  ...
  const loadAllProblemsInfo = useCallback(async () => {
    setLoading(true);
    try {
      const response = await CodeDeploymentAPI.getAllProblemsInfo(deploymentId);
...
`;
  ...
  const loadSpecificProblemInfo = useCallback(async (problemIndex: number) => {
    try {
      const response = await CodeDeploymentAPI.getProblemInfo(deploymentId, problemIndex);
      setCurrentProblemInfo(response.problem_info);
      
      // Load saved code for this problem
      const savedCode = await CodeDeploymentAPI.loadCode(deploymentId, problemIndex);
  ...
  }, [deploymentId, loadAllProblemsInfo, checkChatAvailability]);
  ...
  }, [selectedProblemIndex, allProblemsInfo, deploymentId, loadSpecificProblemInfo]);
  ...
  const saveCodeSilently = async (problemIndex?: number) => {
    ...
    try {
      await CodeDeploymentAPI.saveCode(deploymentId, code, indexToUse);
  ...
  const handleRunTests = async () => {
    ...
    try {
      const result = await CodeDeploymentAPI.runTests(deploymentId, code, selectedProblemIndex);
